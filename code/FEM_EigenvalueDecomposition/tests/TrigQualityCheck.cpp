/*! \file TrigQualityCheck.cpp
 * \brief Implementation of the Poisson equation test with trigonometric solutions.
 ...
 * Author: lebedev
 *
 * Created on 14.07.2016, 18:02:14
 */
...
int main(int argc, char** argv) {
    ...
    std::cout << "%SUITE_STARTING% TrigQualityCheck" << std::endl;
...
dtype f(const dtype& x, const dtype& y) {
    static dtype f = 0;
    f = 0;
    f = SQU(M_PI)*5.0*sin(M_PI*x)*sin(2*M_PI*y);
    return f;
}
...
dtype u(const dtype& x, const dtype& y) {
    ...
    static dtype u = 0;
    u = 0;
    u = sin(M_PI*x)*sin(2*M_PI*y);
    return u;
}

void du(Eigen::Vector2d& v, const dtype& x, const dtype& y) {
    v.setZero();
    v(0) = M_PI*cos(M_PI*x)*sin(2*M_PI*y);
    v(1) = 2*M_PI*sin(M_PI*x)*cos(2*M_PI*y);
}

/*!  \file QualityCheck.cpp
 * \brief Integrated test using Poisson equation on a square with polynomial solutions.
 ...
 * Author: lebedev
 *
 * Created on 12.02.2016, 16:10:20
 */
/* The quality check test is used to compare the results of a numerical solution of
 * the Poisson equation using P_1 and P_2 elements to an analytic solution.
 * Furthermore the convergence behavior of both FE types is investigated.
 */
...
int main(int argc, char** argv) {
    ...
    Eigen::SimplicialLDLT<CSRMat> solverLLT;
...
dtype f(const dtype& x, const dtype& y) {
    ...
    //f = -(12*x^2-6*x)*y*(y-1)-2*x^3*(x-1)
    static dtype f = 0;
    f = 0;
    f = (12 * x * x - 6 * x) * y * (y - 1.0) + 2 * x * x * x * (x - 1.0);
    return -f;
}
